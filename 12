...
class AddressBook(UserDict):
    ...
    def safe_to_file(self,contact):
        with open('phone_book.bin', "wb") as file:
           pickle.dump(contact,file)
        return "Cantacts saved"
    
    def read_contacts_from_file(self,filename = "phone_book.bin"):
        with open(filename, "rb") as fh:
            unpacked = pickle.load(fh)
            return  unpacked 
    
    def try_to_find(self,search_term ):
        search_results = []
        for record in self.data.values():
            if (
                search_term.lower() in record.name.value.lower()
...
if __name__ == '__main__':
    ...
    for page in book:
        ...
        for contact in page:
            contacts = page.get(contact)
            book.safe_to_file(contacts)
            print(f'Original data: \n{contacts}')
            
            loads = book.read_contacts_from_file()
            print(f'Loads data: \n {loads} \n')
    
    search_term = "ja"
    search_results = book.try_to_find(search_term)
    if search_results:
